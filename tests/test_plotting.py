...
def test_plot_ohlcv(ohlcv_data):  # noqa: F811
...
def test_plot_ma(ohlcv_data):  # noqa: F811
...
def test_plot_macd(ohlcv_data):  # noqa: F811
    macd = MACD()
    macd.compute(ohlcv_data)
    plotter = plot(
        ohlcv_data, indicators=[macd], panels=[2], backend="mpl", title="MACD Chart"
    )
    return plotter.fig
...
def test_plot_rsi(ohlcv_data):  # noqa: F811
    rsi = RSI()
    rsi.compute(ohlcv_data)
    plotter = plot(
        ohlcv_data, indicators=[rsi], panels=[2], backend="mpl", title="RSI Chart"
    )
    return plotter.fig
...
def test_plot_combined(ohlcv_data):  # noqa: F811
    ma20 = MovingAverage(window=20, ma_type="SMA")
    macd = MACD()
    rsi = RSI()
    ma20.compute(ohlcv_data)
